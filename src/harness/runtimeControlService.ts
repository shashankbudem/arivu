...
type BrowserModelCandidate = { id: string; config: BrowserTaskModelConfig };

/** Runtime-scoped agent controls shared by the desktop and terminal harnesses. */
export class RuntimeControlService implements RuntimeControl {
  ...
  setAvailableToolNames(names: string[]) {
    ...
    for (const name of names) if (name.trim()) this.availableToolNames.add(name.trim());
  }
  ...
  async setToolState(input: {
    ...
    if (!reason) throw new Error("A reason is required for runtime changes.");
    if (isProtectedTool(name)) throw new Error(`${name} is part of Arivu's control boundary and cannot be disabled by the model.`);
    if (this.availableToolNames.size > 0 && !this.availableToolNames.has(name)) throw new Error(`Unknown tool: ${name}`);
    const target = input.scope === "session" ? this.options.sessionDisabledTools : this.runDisabledTools;
    if (input.enabled) target.delete(name);
  ...
  async selectBrowserModel(input: { candidateId: string; scope: RuntimeControlScope; reason: string }): Promise<RuntimeBrowserModelChange> {
    const reason = input.reason.trim();
    if (!reason) throw new Error("A reason is required for browser-model changes.");
    const candidate = this.candidates.find((item) => item.id === input.candidateId.trim());
    if (!candidate) throw new Error(`Unknown browser-model candidate: ${input.candidateId}`);
    ...
    if (this.activeBrowserModel.fallbacks?.length === 0) delete this.activeBrowserModel.fallbacks;
    if (input.scope === "session") this.options.onSessionBrowserModelChange(cloneBrowserModel(this.activeBrowserModel));
    return {
  ...
  async disabledToolNames(): Promise<string[]> {
    return [
      ...new Set([...(await this.options.readSavedDisabledTools()), ...this.options.sessionDisabledTools, ...this.runDisabledTools])
    ].sort();
  }
}
...
function browserModelCandidates(primary: BrowserTaskModelConfig): BrowserModelCandidate[] {
  ...
  for (const model of models) {
    ...
    if (!seen.has(key)) {
      seen.add(key);
      candidates.push({ id: candidates.length === 0 ? "primary" : `fallback-${candidates.length}`, config: model });
    }
  }
  return candidates;
}
...
function cloneBrowserModel(model: BrowserTaskModelConfig): BrowserTaskModelConfig {
  return { ...model, fallbacks: model.fallbacks?.map(withoutFallbacks) };
}
