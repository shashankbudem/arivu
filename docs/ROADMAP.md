...
- Add rollback helpers and broader patch preview for non-worktree edits. Direct edit artifacts now have draft-revert prompts, and large direct `apply_patch` / `write_file` edits now force a Trusted-mode write review before applying. A richer visual pre-apply patch preview stage for non-worktree edits remains.
