...
- Add deeper per-tool policy drill-downs from Activity rows back to the capability decision that governed each tool call. The Settings policy matrix now includes examples, risk notes, default posture, active-mode reasons, and stricter workspace overrides; read-scope enforcement for `list`, `read`, `search`, and `git_status` is implemented through `read_repo` workspace overrides.
