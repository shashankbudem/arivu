...
- Add stronger semantic plan matching from richer artifacts such as LSP diagnostics beyond the current checklist. One-shot read-only Plan mode, persisted approve/revise/cancel review state, approved-plan drafting, approved-plan-to-worktree handoff, source-plan evidence cards, run-level completion notes, persisted assistant-authored `Completion notes:` with model-authored evidence labels, and conservative changed-file/command/report/PR-check/completion-note matching are implemented.
