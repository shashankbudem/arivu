...
Decision: compact oversized outbound model requests transiently before provider calls, pin the latest user request separately from trimmed tool/browser output, preserve `image_url` parts on pinned multimodal requests while capping oversized text parts, and retry context-length provider failures once with a more aggressive request-only compaction profile.

Reason: a failed run can roll back the visible saved transcript while the in-flight tool/browser transcript sent to the model is still much larger than the saved chat appears. Request-only compaction protects those active runs from context-window failures without rewriting user-visible history or requiring the manual `Compact context` action, latest-request pinning prevents a long todo prompt from being reduced to a short older-message summary after many tool calls, and multimodal preservation keeps screenshot/image prompts useful after compaction.
