...
- Automatic provider capability persistence from observed tool-schema and image-part failures is implemented for auto-mode providers, but proactive provider capability probing is not implemented.
- Doctor does not yet write provider capability observations back to config; it reports checks only.
...
- Add richer remediation and proactive provider capability probes from doctor outcomes.
