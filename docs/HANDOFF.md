...
- Desktop hidden agent browser target plus separate tabbed visible browser window, backed by isolated Electron browser targets and a persistent Arivu browser profile. The visible shell includes scaled device emulation, explicit visible/background tab transfer, element/region review annotations with design previews and composer handoff, history/download/privacy settings, OS-encrypted password and autofill storage, Chrome CSV/Arivu JSON profile import, and unpacked extension management.
...
- Manual context compaction is deterministic and local: it preserves non-compaction system prompts, replaces older visible turns with one hidden system compaction note, normalizes retained tool protocol into plain text, and saves the active session. Desktop exposes it through `Compact context` and `/compact`, the TUI exposes `/compact [n]`, and the CLI exposes `arivu compact <session-id>` with `--recent`, `--entry-limit`, and `--dry-run`. Desktop `/summarize` and TUI `/summarize` optionally ask the active model to summarize older context and fall back to deterministic compaction on failure. The agent also performs transient request-only compaction before provider calls when estimated context is large, pins the latest user request separately from trimmed tool/browser output, preserves `image_url` parts on that pinned request while capping oversized text parts, and retries context-length failures once with more aggressive truncation, so browser/tool loops can recover without rewriting the visible chat.
...
- The agent permits one `web_search` call per user request, then withholds only `web_search` so repeated search loops stop while file, command, browser, and other tools remain available. If a model returns an empty assistant message with no tool calls, the run fails and rolls back to the saved prompt state instead of marking the task completed.
...
- Desktop packaging is configured for macOS, Windows, and Linux. Public signed/notarized installers still require platform credentials and release-time verification.
...
- Browser-task execution now has its own selectable provider/model plus configurable loop cap and delay. Model calls use proxy-level 429/503 backoff, narrowly strip gateway-rejected top-level parameters, retain bounded diagnostics, and temporarily circuit-break persistently failing model endpoints. Browser-task task-run artifacts persist the effective model configuration, step trace, stop reason, and endpoint diagnostics for later review.
...
- Automated GitHub release creation and cross-platform signed installer publication are not implemented; packaging commands and local unpacked/installable builds are documented in `docs/PACKAGING.md`.

## Good next work

Start with the progress log in `docs/IMPROVEMENT_PLAN.md`; its original findings are retained below for history, but all P0 items and the blocking delivery pipeline are complete.
...
- Continue the incremental renderer/main-process decomposition now that blocking CI guards are in place.
