...
- Desktop durable per-query task runs. Each prompt stores run status, selected model/provider, plan/loop metadata, captured assistant plan state, coarse tool capabilities, approval records, tool-call records, and artifacts on the saved session, including screenshots and structured command/report evidence. Activity groups can copy a bounded Markdown audit summary for the run.
