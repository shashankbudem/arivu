...
6. Send or inspect a prompt that runs a command, then confirm the approval prompt and command result row show parser-derived risk/analysis, command text, execution profile/isolation/cwd metadata, exit code, duration, and separated stdout/stderr details.
