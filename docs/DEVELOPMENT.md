...
8. Click the copy-audit icon on the Activity query group and confirm the clipboard contains a Markdown `Arivu task run audit` summary with run id, prompt, status, model/provider, capabilities, tools, approvals, artifacts, verification, and worktree/PR state when present.
