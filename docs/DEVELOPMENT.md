...
- For large direct edit review, create a large `apply_patch` or `write_file` change in Trusted mode outside task-worktree execution and confirm the approval modal says "Write review" with a review-boundary reason before any file is changed. Confirm the same large patch inside a managed task worktree relies on the worktree patch-preview boundary instead of adding the extra direct-edit review prompt.
