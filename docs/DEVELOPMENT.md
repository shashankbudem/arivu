...
- Tool-call activity is grouped by the triggering user query. Verify a prompt that uses multiple tools shows one expandable tool-run marker in the chat transcript and one matching expandable query group in the Activity panel. Use the Activity group's copy-audit icon to copy a Markdown audit summary for that run.
