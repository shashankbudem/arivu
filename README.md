...
- Browser tools include `browser_state`, `browser_select_tab`, `browser_open`, `browser_screenshot`, `browser_task`, and lower-level snapshot, console, click, type, scroll, and select helpers. `browser_task` is the preferred multi-step in-page interaction path.
- The browser-task page agent is capped at 100 loops by default and waits 35 seconds between loops to avoid hammering its LLM provider. Its default wall-clock budget is 70 minutes; runs remain cancellable, and individual tool calls can request smaller loop or time budgets.
