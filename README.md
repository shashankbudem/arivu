...
- The [Grok Build](https://github.com/xai-org/grok-build)-inspired TUI keeps the transcript full-width, folds tool calls into compact inline rows, and exposes expanded tool arguments/results in an on-demand Activity drawer. `Ctrl+P` opens the searchable command palette, `Ctrl+G` toggles Activity, `Ctrl+S` opens the saved-session picker, and `Ctrl+X` shows all shortcuts. Local slash commands include `/status`, `/diff`, `/activity`, `/compact [n]`, `/sessions [n] [--pick] [--search text] [--workspace text] [--pinned|--unpinned] [--project|--standalone]`, and `/resume <id>`. Prompts entered during an active turn are queued instead of discarded.
