...
- Direct-edit artifacts include bounded unified diffs for `apply_patch` and bounded new-content previews for `write_file`, so non-worktree edits remain inspectable in Activity after reload or compaction. Large direct `apply_patch` / `write_file` changes are treated as risky write reviews even in Trusted mode unless a managed task worktree already owns the review boundary. Those rows can also draft a revert prompt seeded from the saved edit evidence.
