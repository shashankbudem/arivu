...
`arivu doctor` validates the configured API key, model listing, selected model, basic chat completions, streaming, tool calling, and the active web-search provider. When no model API key is configured, model network checks are skipped. Keyless Bing RSS needs no credential probe. Desktop Settings doctor also saves Tool calling as `Disabled` for an auto-mode saved provider when the forced tool-call probe proves unsupported.
...
The Browser settings can also enable a dedicated `nvidia/LocateAnything-3B` visual-grounding model through a saved OpenAI-compatible provider, such as a local vLLM endpoint. This model is separate from the Browser task LLM: Page Agent still reasons and uses current DOM indices first, while `locate_and_click` is exposed only as a last-resort fallback for pixel-only controls. The trusted main process captures the current viewport, sends the official `Point to: …` prompt, converts LocateAnything's normalized 0–1000 output to viewport coordinates, rejects missing, ambiguous, or stale results, and dispatches one click. The released model is licensed for non-commercial research and its official runtime targets NVIDIA GPU/Linux systems; Arivu does not bundle or silently download its weights.

Apple-silicon Macs can serve the community `yuuko-eth/LocateAnything-3B-GGUF` conversion through its `mtmd-grounders` llama.cpp fork. Use both the recommended Q4 model and the BF16 vision projector, enable Metal with `-ngl 99`, and pass `--special`: without that last flag, llama-server removes the coordinate control tokens Arivu needs. For example:
...
llama-server \
  ...
  -ngl 99 --special \
  --alias nvidia/LocateAnything-3B
```

Configure its Arivu provider as `http://127.0.0.1:8000/v1` with model `nvidia/LocateAnything-3B`. This is a community Apple-silicon port rather than NVIDIA's official runtime, so pin and test the fork/model revisions before distributing it.

On a supported NVIDIA GPU/Linux host, serve the model with the model card's vLLM flow:

```bash
vllm serve "nvidia/LocateAnything-3B"
```

Then add an Arivu provider with base URL `http://127.0.0.1:8000/v1` (or the reachable GPU host), model `nvidia/LocateAnything-3B`, and select that provider under Browser > Visual grounding.
...
Settings > Integrations has a web-search provider manager for Tavily, Brave Search, Exa, Serper, and keyless Bing RSS. One provider is active at a time, and the same selection is used by both the main agent and the in-page browser agent. Each profile keeps its own endpoint and API key. Existing `ARIVU_TAVILY_API_KEY`, legacy `SHANKINSTER_TAVILY_API_KEY`, `TAVILY_API_KEY`, and saved `tavilyApiKey` configurations migrate into a Tavily profile automatically. With no configured provider, Arivu creates a Bing RSS profile. Web search still requires approval before a query leaves the machine; news-like Bing queries use Bing News RSS, refresh stale generated years, and decode Bing redirect links.

Browser control:

- The terminal TUI and one-shot CLI use the local `browser-use` command as a direct Chrome/CDP adapter. Install it with `uv tool install browser-use`, then run `browser-use --doctor`. Set `BU_CDP_URL` only when Arivu should attach to a particular Chrome debugging target; otherwise Browser Use manages its own session.
- Terminal sessions expose direct `browser_state`, `browser_select_tab`, `browser_open`, `browser_snapshot`, `browser_screenshot`, `browser_click`, `browser_click_at`, `browser_type`, `browser_scroll`, `browser_select_option`, and `browser_execute_javascript` tools. `browser_open` creates a new agent tab by default so it does not overwrite the user's current tab. The terminal backend does not advertise or start `browser_task`; Arivu's main model remains the only reasoning/action loop. Browser Use does not currently provide retained console history, so terminal `browser_console` returns an explicit empty result.
...
- Desktop browser tools include `browser_state`, `browser_select_tab`, `browser_open`, `browser_screenshot`, `browser_task`, `browser_execute_javascript`, and lower-level snapshot, console, click, type, scroll, and select helpers. On desktop, `browser_task` is the preferred multi-step in-page interaction path. Its default wall-clock ceiling is 70 minutes; explicit budgets below 10 minutes are raised to 10 minutes so the configured 35-second loop pacing and slow provider responses cannot terminate a healthy task after only one or two steps. Completed tasks still return immediately.
...
- The in-page agent behind `browser_task` always has its own `search_web` tool, unconditionally (unlike the opt-in script tool). It routes through the same per-task loopback proxy as the in-page LLM calls, authenticated with that task's own bearer token, and reuses the active web-search profile from Settings. Its system instructions steer it to reach for this when it does not know how a specific field, control, or workflow on the current site works, not as a first resort. The main agent can also help directly: if a `browser_task` trace shows the same interaction failing repeatedly, `web_search` for how the site actually works before re-running with a clearer instruction.
- When visual grounding is configured, Page Agent additionally receives `locate_and_click`. It captures a fresh viewport and uses `nvidia/LocateAnything-3B` to ground one visually unique target. DOM actions remain primary; the tool refuses multiple matches and rechecks the URL, frame tree, viewport, and scroll position before clicking so coordinates from old pixels cannot land on a changed page.
...
- The TUI is a native Rust frontend built with the same Ratatui, Crossterm, and pinned [`xai-ratatui-inline`](https://github.com/xai-org/grok-build) stack as Grok Build. Finalized messages flow into terminal-native scrollback while only the live response, status, and compact rounded composer stay pinned. Type `/` or press `Ctrl+P` for fuzzy slash completion, `Ctrl+G` for complete tool activity, `Ctrl+S` for saved sessions, and `/help` for every shortcut. Commands include `/status`, `/diff`, `/runs` (durable task evidence and queued prompt IDs), `/undo <task-run-id>` (direct-workspace checkpoints), `/steer <queued-prompt-id>`, `/activity`, `/continue`, `/compact [n]`, `/summarize`, `/sessions [n] [--pick] [filters]`, `/resume <id>`, `/new`, `/rename <title>`, `/pin`, `/delete`, `/clear`, and `/exit`. TUI task runs persist model routing, tool/approval evidence, verification, and recovery state; prompts entered during an active turn are stored in the session queue and can steer at a safe model boundary. Explicit `!` shell commands stay outside session history and remain foreground-only.
