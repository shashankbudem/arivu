...
- Direct-edit artifacts include bounded unified diffs for `apply_patch` and bounded new-content previews for `write_file`, so non-worktree edits remain inspectable in Activity after reload or compaction.
...
Inside the TUI, use `/sessions` to list recent saved sessions and `/resume <session-id>` to switch into one without leaving the terminal UI.
